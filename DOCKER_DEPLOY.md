...
- `AI_LOCALBASE_IMAGE_TAG`：预构建镜像版本，生产环境建议使用具体 tag，例如 `v1.4.10`，不要依赖 `latest`
- 生产 Compose 默认使用已发布的固定 `v1.4.10` 镜像；升级或回滚时通过 `AI_LOCALBASE_IMAGE_TAG` 显式切换。本地源码修改请使用开发或本地构建编排验证。
...
```bash
# 拉取指定版本镜像
export AI_LOCALBASE_IMAGE_TAG=v1.4.10
