...
AI_LOCALBASE_IMAGE_TAG=v1.4.11 docker compose -f docker-compose.prod.yml up -d
...
- `AI_LOCALBASE_IMAGE_TAG`：预构建镜像版本，生产环境建议使用具体 tag，例如 `v1.4.11`，不要依赖 `latest`
- 生产 Compose 默认使用已发布的固定 `v1.4.11` 镜像；升级或回滚时通过 `AI_LOCALBASE_IMAGE_TAG` 显式切换。本地源码修改请使用开发或本地构建编排验证。
...
- `API_REQUESTS_PER_MINUTE`：每个高成本 HTTP 资源类别按主体/IP 计算的每分钟请求上限，各类别分别计算，默认 `120`
...
```bash
# 拉取指定版本镜像
export AI_LOCALBASE_IMAGE_TAG=v1.4.11
